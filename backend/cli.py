...
from backend.analytics import analyze_problem
from backend.fetcher import fetch_solved_problem
...
def build_parser() -> argparse.ArgumentParser:
    ...
    fetch.add_argument("--pretty", action="store_true", help=argparse.SUPPRESS)
    ...
    analyze.add_argument("--pretty", action="store_true", help=argparse.SUPPRESS)
...
def main (argv: list[str] | None = None) -> int:
    ...
    try :
        if args.command == "fetch":
            ...
            payload = {"analysis": analyze_problem(args.problem_id, cache_dir=args.cache_dir)}
        else:
